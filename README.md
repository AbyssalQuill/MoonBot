...
| `dshCompaction` | 上下文治理：`thresholdRatio`（默认 0.12，1M 窗口 ≈ 12.6 万 token 才压缩）、`retainRatio`（默认 0.03，最近 3% 逐字保留）、`toolResultMaxChars`（默认 8192，超长工具结果剪成「头 60% + 标记 + 尾 20%」）。**阈值必须显著高于单次请求的上下文基线**（system + 工具清单 ≈ 2.75 万 token 的固定开销）：2026-09-20 线上实测，阈值 0.06（= 6.3 万）被固定开销顶穿，114 个模型步里触发了 46 次摘要，每步之间多花 15~20 秒、前缀缓存全废。桥的下限是 0.08，低于它会被自动夹回并写日志 |
