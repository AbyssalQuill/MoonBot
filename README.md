...
每条消息进来都要先过一次唤醒判定（`core/mux.js` + `core/social-flow.js`），只有判定为"该醒"才会真正调用模型（不醒的消息仍然入库，作为上下文与学习素材）：

| 唤醒类型 | 触发条件（简述） |
|---|---|
| `bootstrap` | 新会话的第一轮：注入完整提示词（preset + 人设）与最近消息窗口 |
| `private` | **私聊消息**（主人私聊还有额外特权与专属行为） |
| `atMention` | 群里被 **@** 或被点名（昵称/别名命中） |
| `poke` | 被**戳一戳** |
| `keyword` | 命中配置的触发关键词 |
| `probability` | **概率主动**：按配置概率在"有人在聊"的群里主动开口 |
| `replyCheck` | 回复前/后的**复查**（对方又补了一句、或该不该继续说） |
| `proactiveCheck` | **主动闲聊**定时检查（私聊/群聊各自的概率与间隔） |
| `timeout` | 等待窗口到期后的收尾判断（不再追问、去睡） |
