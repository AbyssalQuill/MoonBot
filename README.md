...
当前版本 **1.2.3**，产品名 MoonBot Pro，对外安装包为 `MoonBot Pro Setup.exe`。更新记录见 [CHANGELOG.md](CHANGELOG.md)，1.0.0 那版的发布说明留档在 [docs/release-1.0.0.md](docs/release-1.0.0.md)。
...
- **点顶部「保存」（写整份 `config.json`）**：常用设置里的全部字段 —— 模型/厂商、NapCat 地址与路径、基础与会话、允许/拒绝名单、唤醒与潜水、发送节奏、上下文与轮换、上下文治理、主动闲聊、等待、打字等待、表情包参数（收藏表情那一套）、静默群聊、投递与回合、好友申请、Word 额度，以及「工具与规则」里的工具开关与工具 schema 精简；内置表情包的总开关 `social.meme.enabled` 也在这一份里（**它目前只能在「JSON 进阶」里逐键改**，表情包卡片本身没做勾选框），而**包本身与角色绑定**由下面那张卡自己写盘。
...
| `/start`、`start` | 解除群聊静默 |
| `/set active` | 本会话全天活跃：群里说啥都接，并清除该会话的活跃时段 |
| `/set active 09:00-01:00` | 只在**这段时间**活跃，其余时间潜水（只回 @ 与点名）。跨午夜写作 `09:00-01:00` 即可 |
| `/set diving` | 本会话全天潜水：只回 @、被点名与私聊 |
| `/set diving 00:00-21:00` | **这段时间**潜水（只回 @ 与点名），其余时间照常活跃 |
| `/set mode active`、`/set mode diving` | 上面两条不带时段的等价老写法 |
| `/set sleep 01:00-06:00` | 设置作息时段，分隔符可用 `-`、`~`，时间分隔符可用半角或全角冒号 |
...
| `/op` | 显示当前管理员与用法 |
| `/op <QQ 号或昵称>` | 设置管理员 |
| `/op del <QQ 号或昵称>` | 取消管理员，也可写作 `/op remove` |
| `/like <QQ 号> [次数]` | 点赞，次数范围 1 到 10 |
| `/slang learn`、`/slang stop` | 立即学习黑话 / 停止在跑的学习任务 |
| 其他 `/` 开头 | 交给模型处理 |

斜杠命令**一律英文**（中文说法如"转活跃""学一下黑话"交给模型自然处理，由它自己调工具）；只有管理员（`ownerQQ` 与 `adminQQ`）可用。
...
| 写法 | 作用 |
| --- | --- |
| `/slang learn` | 立即执行一次黑话学习 |
| `/slang stop` | 停止进行中的黑话学习与研究 |
| 其他 `/slang` 内容 | 返回用法说明 |

中英混写的旧写法（`/slang 学习`、`/slang 停止`）以及历史简写（`/slanglearn`、`/slangstop`）都已不再识别，只回用法说明。
...
| `dshCompaction` | 上下文治理：`thresholdRatio`（默认 **0.08**，1M 窗口 ≈ 8.4 万 token 才压缩）、`retainRatio`（默认 0.02，最近 2% 逐字保留）、`toolResultMaxChars`（默认 8192，超长工具结果剪成「头 60% + 标记 + 尾 20%」）。**这一项直接决定每条消息多少钱**：从 `state/token-usage.jsonl` 逐条算下来花费几乎正比于上下文大小（缓存命中价约为全价 0.1，而每次请求未复用的只有一两千 token）—— 实测 0~40k ≈ **0.41 分/条**、40~70k ≈ 0.56、70~90k ≈ 0.79、90~110k ≈ 0.82、110~140k ≈ **1.04 分/条**。所以阈值两头都不能偏：**低于 0.08** 会被固定开销（system + 78 个工具 ≈ 2.75 万 token）顶穿 → 每一步都压缩一次（2026-09-20 实测 0.06 时 114 步触发 46 次摘要、每步多花 15~20 秒）；**高于 0.10** 则上下文长期停在十几万 → "一句话 1 分钱"。桥的下限是 0.08，低于它会被自动夹回并写日志 |
