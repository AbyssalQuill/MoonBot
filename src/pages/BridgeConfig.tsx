...
/** 进阶项说明（点 ⓘ 展开），只给对新手不友好的项加 */const HELP: Record<string, string> = {
  ...
  thresholdRatio: '「触发比例」：上下文用到**模型窗口的百分之多少**就开始治理（默认 0.08 = 8%）。'
    + '窗口 1M 的模型 ≈ 8.4 万 token 触发；窗口 128k 的模型 ≈ 10k token 触发 —— 同一个比例换模型自动缩放，不用手改。'
    + '**这一项直接决定每条消息多少钱**：从 token 用量逐条算下来，花费几乎正比于上下文大小 ——'
    + '实测 0~40k ≈ **0.41 分/条**、40~70k ≈ 0.56、70~90k ≈ 0.79、90~110k ≈ 0.82、110~140k ≈ **1.04 分/条**'
    + '（缓存命中价只有全价的约十分之一，而每次请求未复用的只有一两千 token，所以"上下文多大"几乎是唯一变量）。'
    + '调大 = 记得更多、但每条更贵；调小 = 每条更便宜。'
    + '**别低于 0.08（8%）**：固定开销本身就有 **2.75 万 token**（system 提示词 5.4 万字符 + 78 个工具的 8.1 万字符），'
    + '阈值一旦被它顶穿就变成**每一步都压缩一次** —— 每次压缩额外发一次"读完整段上下文写摘要"的模型请求、并改写'
    + '会话历史（prompt 前缀缓存随之作废，下一步只能全量重读）。2026-09-20 线上实测：0.06 时 114 个模型步里触发了'
