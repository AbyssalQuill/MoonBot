...
export interface ServiceInfo {
  ...
  reachable: boolean;
  status?: number;
/** 2026-09-14local = 本机那套；remote = 服务端（经 SSH 隧道）那套。两组并列，绝不互相顶替。 */
  scope?: 'local' | 'remote';
...
/** 安装位置体检（装在 Program Files / 同步盘等风险位置的提醒；正常安装为空） */
  warnings?: string[];
/** 2026-09-17本机与服务端同时有 NapCat 在线（同一个 QQ 号两处登录，会被腾讯互踢）；界面要显著提醒 */
  dualNapcat?: boolean;
...
/** 2026-09-22启动时自动连接服务器（默认开；SSH 配置页可关） */
...
/** NapCat WebUI 登录 token（默认 truefriend） */
  webuiToken?: string;
/** 关闭管理器界面/退出管理器进程时，是否结束本次这个启动器拉起来的 NapCat（默认 true）。
