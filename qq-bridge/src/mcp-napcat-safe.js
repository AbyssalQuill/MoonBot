...
if (cfg.social?.tools?.sendRich !== false) {
  registerTool(
    ...
    // 不传就空白（手机端尤其明显）。所以音乐卡一律要带 image（用 qq_music_search 返回的 cover 原样传）。
      ...
      image: z.string().optional().describe('REQUIRED FOR MUSIC CARDS (163 / qq / custom) - pass the cover URL copied verbatim from the cover field of qq_music_search (already https + 300x300). A music card sent WITHOUT image shows a BLANK cover on mobile QQ; this is a hard rule. Ignored for video/location/contact/dice/rps. The bridge uses this value as-is and only falls back to its own lookup when it is missing.'),
