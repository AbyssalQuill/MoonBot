// 自测脚本：不依赖 NapCat / QQ，只验证 DSH 侧链路——
//   1. 连接 DSH Web API
...
async function main() {
  ...
  const api = new NodeApiClient(baseUrl, undefined, { dshLogFile: process.env.DSH_ISOLATED_LOG_FILE || '' });
  ...
  // rc.1 无全局 mux：events.mux 需要声明要订阅的会话集合
  api.setSessionIdSource(() => [sessionId]);
