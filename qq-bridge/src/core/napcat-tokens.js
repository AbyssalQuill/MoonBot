...
let cfgRef = null;
export function initNapcatTokens(cfg) { cfgRef = cfg; }
...
export function napcatTokenStatus() {
...
function backupDir(dir) {
  ...
  try {
    ...
    const names = ['webui.json', ...onebotFiles(dir).map((p) => path.basename(p)), ...protocolFiles(dir).map((p) => path.basename(p))];
...
function restartContainer() {
  ...
  const t0 = Date.now();
  const r = spawnSync('docker', ['restart', '-t', '30', name], { timeout: 180000, encoding: 'utf8' });
