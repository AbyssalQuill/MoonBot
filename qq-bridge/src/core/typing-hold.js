// 私聊「不抢话」的等待 / 插话决策（2026-09-15 需求）
//
// 目标：私聊里模型回复要先看对方打字状态，等 ta 打完再回；对方不停发消息时，
// 打字状态应当是连续的；再用概率投骰子决定要不要打断这个状态 —— 既能不抢话，又能"智能接话"；
// 等待期间到的消息全部排队，最后合并成一次注入（省注入轮数）。
//
// 这里只放纯决策（不碰状态、不发网络），便于单测；状态维护在：
//   · core/events-aux.js   —— QQ 的 input_status 事件 → st.peerTypingUntil / peerTypingSince
//   · core/social-flow.js  —— 私聊来消息时续上打字窗口（QQ 的输入事件不可靠，不停发消息=一直在打字）
...
//
// 现场（bridge.log 14:05:39 / 管理端 13:39:59）：消息在模型还在跑这一步时到达 →
// 上面那个 typingHoldDecision 判"等"→ 在途注入被推迟 → 等待期间那一轮跑完了 →
// 投递时"没有正在跑的模型回合"→ 落回完整唤醒（日志原文 `唤醒 private:*（private）`），
// 也就是"被塞进了下一个唤醒"，而不是注入当前这一轮。
//
// 机理上有三处分寸不对，这里用一个纯函数把它们定死（判据集中一处，便于单测）：
//   ① 模型还在生成、这一轮一条都还没发出去 → 根本没有"抢话/碎裂"的问题（对话窗口里还没有气泡），
//      等下去只会把这条消息拖过这一轮 ⇒ 立刻注入当前轮（noReplyYet）。
//   ② "打字保持中"（对方在连发 + 本回合已经发过气泡）才允许等，而且只允许短暂等
//      （STEER_IN_TURN_DEFER_MAX_MS 硬上限；到点必投，绝不把消息拖过这一轮）。
//   ③ 允许等的时候必须给出补投时刻（retryAtMs = 打字窗口结束或延迟上限，取先到者），
