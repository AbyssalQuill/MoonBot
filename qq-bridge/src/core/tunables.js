...
export const TUNABLE_SPECS = [
  ...
  // 2026-09-15：活跃模式的随机搭话概率：转活跃时用它，不再沿用潜水那套 0.05（当时的反馈是"活跃跟潜水没区别"）
  ...
  /* 2026-09-24：上限 320 → 1000。原上限是 2026-09-22 为"唤醒后响应太慢"（当时线上被调成 650/15000）
     设的护栏，结果把主人 23:32 亲口定的"一个字 500 毫秒"直接截成 320：调了不生效、也不报错，
     界面上只看到"保存成功"（主人原话"你没填成功"）。现在的护栏交给 linearCapMs（默认 6000，
     也是主人自己定的）做总量封顶，perChar 只保留 60~1000 的合理性区间。 */
  { key: 'sendPerCharMs', path: ['social', 'send', 'linearPerCharMs'], type: 'dur', min: 60, max: 1000, label: '每个字的打字时间', desc: '每个字算多少毫秒（默认 150；合法区间 60~1000）。主人说"打字慢点"就调大（300~600），"快点/太慢了"就调小（80~120）。单条气泡的总等待另由「两条气泡最大间隔」封顶' },
  ...
  // —— 私聊「不抢话」：等对方打完字再回 + 概率插话（2026-09-15 需求，见 core/typing-hold.js）——
  ...
  // —— 上下文治理（2026-09-19 需求"一个会话永久用、别让上下文堆积"）——
...
export function tunableListItems() {
  /* 2026-09-24：把合法区间一并吐出来。起因：主人让"每个字打慢点"，把 500 写进 perCharMs，
     被 spec.max=320 悄悄夹回 320 —— 模型拿到的回包只有 value:320，没有任何"被夹过"的迹象，
     于是它对主人说"改好了"，主人看到的是"还是连发"。现在每项都带 min/max（数值型）与
     type，模型改之前就能看出这个键能取多大，改完也能拿 requested/applied 对照。 */
  return TUNABLE_SPECS.map((s) => ({
    key: s.key, label: s.label, type: s.type, value: readCfgPath(s.path), desc: s.desc,
    ...(s.type === 'dur' || s.type === 'float' ? { min: s.min, max: s.max } : {}),
  }));
}
 ...
 * 解析隔离 DSH 的 home 目录。
 *
 * 2026-09-12 修「管理端改的模型配置无法默认到 DSH 里」：这里原来写死 `~/.qq-bridge-manager/dsh-isolated-home`，
 * 而管理器实际启用的隔离实例是 `dsh-isolated-home-official`（见管理器 config.json 的
 * `instances.dshIsolated.isolatedHome`，DSH 端口 10721 就是它）。于是模型改动全写进了那个早就没人读的旧目录，
