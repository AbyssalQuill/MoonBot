...
export function loadConfig() {
  ...
  const cfg = {
    ...
    dshCompaction: {
      ...
      /* 【2026-09-20 线上实测后从 0.06/0.012 抬到 0.12/0.03】
       * 现场（VPS 私聊会话 session-73c05519，10 回合 114 步）：阈值 0.06 × 1M 窗口 = 62.9k token，
       * 可这个会话**每次请求的上下文基线就是 68.7k token**（system 5.36 万字符 + 78 个工具 8.11 万字符
       * ≈ 2.75 万 token 的固定开销，加上历史），于是**阈值被固定开销自己顶穿**：
       *   · 114 步里触发了 46 次摘要（compaction/start 71 次），几乎每一步都压一次；
       *   · 每次压缩 = 一次额外模型请求（把上下文整段读一遍写摘要，实测单次 3~6.9 万 token 未命中缓存）
       *     + 改写会话历史（前缀缓存全废，下一步只能全量重读）→ token-usage 里未缓存输入 116.9 万；
       *   · 体感就是"每步之间多出 15~20 秒"，一个搜索回合拖了 6 分钟（主人看到的正是"又慢又贵"）。
       * 现在 0.12 × 1M ≈ 12.6 万 token：正常会话（~69k）根本不触发，压缩只在真正长会话才发生；
       * 保留 3% ≈ 3.1 万 token，压缩后上下文 ≈ 3.1 万 + 固定 2.75 万 = 5.9 万，**远低于阈值**，
       * 不会再出现"压完立刻又压"。 */
      thresholdRatio: 0.12,      // 上下文用到窗口的多少比例就开始治理（0.12 × 1M ≈ 126k token）
      retainRatio: 0.03,         // 最近多少比例的上下文**逐字保留**（必须小于 thresholdRatio）
