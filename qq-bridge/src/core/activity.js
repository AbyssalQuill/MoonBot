...
export function loadActivityWindows() {
  // 仅在文件缺失(首次运行)时 seed 默认时段；文件存在但为空(主人已清除全部时段)绝不再 seed，
  // 否则主人清空时段后一重启旧群的 12-14/18-次日1 时段又“复活”。
  ...
  // 【2026-09-12 去硬编码】这里原来 seed 的是主人自己那两个群的 12-14 / 18-次日1 时段，
  // 也就是把**主人的群号写死在代码里**（打包给别人时会被夹带，别人还会莫名地在"某些群只在特定时段活跃"）。
...
// 主人已把该群转为全天活跃，时段提醒只会让模型在窗外继续潜水/睡到下一时段，
// 造成“转活跃了怎么还是有时间段”的假象。
