...
//   target = "next-step";
// 这一步跑完、没有活着的工具调用、next-step 为空时回合会关；但关闭前会 await 一次
// `agent/turn-stopping` 插件钩子，并在钩子返回后重新检查 next-step。
// 所以只要钩子里肯等，桥就能趁这段时间把新消息 steer 进 next-step —— 回合不关，直接跑下一步。
// 相比之下 `mode:'queue'` 投的是 next-turn，那必然多出一整轮（含整包 prompt 重发）。
//
// ── 分工（插件只当时序闸门，桥负责 steer）────────────────────────────────────
//   插件：POST /api/qq/turn-hold 干等桥的回答；桥要它继续持有就立刻再问一次（见下"分段持有"）。
//   桥：决定"继续持有 / 放行关闭"，需要继续时用既有已验证的 steerIntoRunningTurn() 把消息塞进去。
//
...
const POLL_MS = 200;
// 吊住回合期间没有任何桥 / DSH 事件，turn-guard 的"静默 180s 判卡死 / 总时长 360s"计时器
 ...
 *  提出来是因为它现在有两个调用点：回合钩子（handleTurnHold）与步边界发车（flushStepBatch）。
 *  wake-send.js 里还有一份等价的内联判据（holdEligible）；改这里时记得同步，别让两处漂移。 */
...
async function holdLoop({ key, sid, st, turn, t, cfg, shouldAbort }) {
  ...
  /* 2026-09-22：报障「出了 OK 之后界面还挂着『深度求索中…15 分 02 秒』」。
   ...
   * 把对方连发的那几句并在同一个回合里（他几秒内接着说的下一句），而"已经答过一轮、又静默了两分钟"
   * 这种状态下继续持有只剩下副作用（界面骗人、白白占着一个在跑的回合）。
   * 所以加一个更短的判据：本回合已经说过话（turnHasBubble）+ 静默超过 answeredIdleCloseMs → 收回合。
   *
   * 2026-09-23：需求「私聊回了就立刻停，别再挂深度求索中」→ 支持显式 0。
   * 语义定义（answeredIdleCloseMs）：
   *   · 0    = 已经答过、且手上没有待交付消息 → 立刻放行关回合（不等静默窗口）
   *   · >0   = 答过之后再静默这么久才放行（原行为，默认 90000）
   * 注意：旧写法 `Number(x) || 90000` 有两个坑叠在一起：显式的 0 是 falsy，会被 `||` 当成"没配"
   *    而还原成 90000；而下面的守卫又写成 `answeredIdleMs > 0`，于是就算把 0 传进来也只会让这条
   *    判据永不触发，直接退回 idleCloseMs（30 分钟）—— 想调短的人反而把回合挂得更久。
   ...
   * 体感上的「卡一段」就是它（不是模型慢）。改成指数退避：头两次仍很快（打字快的人不受影响），
  ...
  /* 2026-09-23：需求「主动唤醒的回合还会挂住深度求索中 → 立即收尾」。
   * 保持循环存在的唯一理由，是把对方连着发的那几句并在同一个回合里。
   ...
   * 尤其是模型这一轮选择不开口时（主动机会检查后决定不说话），连 turnHasBubble 都是 false，
   * 连 'answered-idle' 那条都命中不了，必然拖到 30 分钟 —— 这正是线上看到的现象。
  ...
  while (true) {
     ...
     * 待交付批次非空时不收：那是刚发的新消息，必须留给下面的 steer 合并进本回合。 */
    ...
    /* 2026-09-22：修「私聊 4 分半不回复」。批次可能不是本循环投出去的：wake-send 的即时 steer
     ...
     * 注意：只在没有待交批次时放行：手头还有没塞出去的消息时，下面那段 steer 逻辑必须照跑
    ...
    // 合并注入把消息攒在 unread 里、不即时投（wake-send.js 的"turn-hold 托管"分支），而它们
    // 在钩子开始之前就到了 —— 用旧判据（baseline 是钩子开始时刻的 lastUnreadSeq）一条都看不见，
    // 保持循环只会干等到预算用完，消息反而被卡住（即"等下一次"这个抱怨的来源）。
    // 判据共用 wake-send.js 的 collectMidTurnBatch，和即时注入那条路同一份规则，不会漂移。
    ...
    if (batch.length) {
      ...
      if (ok === 'typing-defer') {
        // 2026-09-16 打字窗合并：对方还在打字 → 这一批继续攒：不投、不改水位、不关回合。
        // 为什么不能走下面的 finish()：那会让 DSH 收尾这一轮、消息退回下一轮唤醒 —— 需求是
      ...
      if (ok !== true) {
        ...
        // 而且有些消息在本轮唤醒正文里就已经展示过（turnSeenUnread），不必再注入。
        // 实测 21:50:23 就是这样：保持循环报 steer-failed，但消息其实已经进了本回合。
        // 2026-09-15 合并注入：触发判据换成 collectMidTurnBatch 之后，`batch` 里的行按定义
      ...
      // 正是这个项目吃过四次的那类"静默失败"。所以记这一笔之前必须验证真的投出去了：
 ...
 * mux.js 每收到一条会话事件 `step/end`（dsh-agent-loop/lib/index.js:558 在每个模型步末尾 append）
 * 就调一次这个函数 —— 把这一步里攒下的消息合成一个 [Mid-turn] 块注入。
 *
 * 为什么光有保持循环不够：保持循环活在 `agent/turn-stopping` 钩子里，而那个钩子只在"这一步可能就是
 ...
 *   · 却仍然只产生一个 [Mid-turn] 块（这一步里攒下的消息全在这一个块里）。
 * 不满足条件时静默返回 false：这个函数每一步都会被调一次，没攒下消息是绝大多数情况，不能打日志刷屏。
 *
...
export async function flushStepBatch({ key, sid, turn, cfg } = {}) {
  ...
  if (ok === 'typing-defer') {
    // 2026-09-16 打字窗合并：对方还在打字：这一步不发车。消息仍在 unread 里（没标"已给"），
