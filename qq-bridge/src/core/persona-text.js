 ...
 *
 * 现在的规则：
 *   - 只输出一段完整的中文介绍，不出现 `昵称:` / `性格:` / `备注` 这类字段名；
...
/**
 * 清掉历史上写进 `profiles.notes` 的「人格学习:…」段落（那是学习结果，不是用户写的备注）。
 * 旧版每次学习都把摘要追加到备注里，界面上看到的就是两段几乎一样的"备注"。
 * 返回清理后的备注：用户自己写的部分保留，学习段落删掉；全是学习段落就返回空串。
 */
