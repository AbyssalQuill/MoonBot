...
//
// 为什么要这么做（2026-09-19：一个会话永久用下去，但别让上下文堆积，省掉切新会话的首轮 token）
// 桥侧不能自己去删 DSH 的会话历史。依据（读过 DSH 0.1.2-rc.1 源码，不是猜的）：
...
// 受支持的裁剪路径只有一条：由 DSH 自己通过 surface `replace` 改写历史 —— 也就是
...
//   · profile 的 `patchReload: live` 会同时监视 home 级 patch 文件 → 改完不必重启 DSH；
 ...
 * ② 2026-09-20 上午：阈值 0.06，可固定开销就有 2.75 万 token（system 5.36 万字符 + 78 个工具
 *    8.11 万字符），加上历史，62.9k 的阈值被顶穿 → 114 个模型步触发 46 次摘要，每步之间多花 15~20 秒；
 * ③ 2026-09-20 晚：阈值抬到 0.12 后不再每步压缩，但上下文长期停在 ~11.7 万 token，实测
 *    为"一句话 1 分钱，不划算" —— 于是收到 0.08。
 ...
 *    ⇒ 最省的是少压缩：0.08 → ¥3.34/天、0.12 → ¥2.67、0.16 → ¥2.53、0.18 → ¥2.53、0.25 → ¥2.66
...
export function normalizeCompaction(raw) {
  ...
  // 以前缺省 1500 会把它们剪成 900+300，模型只看到零头（2026-09-19 收到反馈后改）。
  ...
  /* 2026-09-19：摘要一律用会话主模型 / 全局服务商 —— 不再支持单独指定
...
export function buildCompactionRows(p) {
  const lines = [];
  /* 2026-09-19 实测踩坑：web profile 下这两行默认是禁用的，必须显式 disabled: false
  ...
  /* 不写 summarizationProvider / summarizationModel：留空 = 跟主模型（复用同一服务商与提示词缓存）。
     2026-09-19：统一用全局语言模型服务商，改这里（lib/dsh-compaction.js）时别再加回来。 */
