// 表情包体系（default模式）——本地表情知识库与策略提示纯函数。
//
// 职责：
// - state/stickers.json 的读写与字段归一化
// - 把 NapCat `fetch_custom_face_detail` 返回的 QQ 收藏表情合并进本地库
...
// 两轮匹配：先精确（id/resId/md5），再模糊（url 包含）。此前单轮 .find 会让"靠前的条目 URL 恰好
// 包含 ref 子串"抢走后面的精确命中 → 发错表情 / 备注记到别的条目上（findSticker 是发送与备注共用入口）。
export function findSticker(entries, ref) {
  ...
  const exact = list.find((e) => e && (e.id === id || e.resId === id || (e.md5 && e.md5 === md5)));
...
export function buildStickerStrategyHint(rate) {
  ...
  if (Number.isFinite(r) && r >= 0 && r <= 1) {
    const band = r < 0.15 ? '低' : r <= 0.45 ? '中' : '高';
    const pace = r < 0.15 ? '大约每 8~12 轮才来一张，宁缺毋滥' : r <= 0.45 ? '大约每 2~4 轮来一张，穿插在接梗/赞同/无语/输了赢了等场合' : '大约每 1~2 轮就可以来一张，气氛热闹时更密';
  ...
  return [
    ...
    '- 频率：普通闲聊大约每 2~3 轮就可以来一张；接梗、被夸、怼人、赞同、无语、赢了/输了、别人发图这些场合尽量跟一张，热闹/玩梗时可以更密，只要别连续刷屏。',
