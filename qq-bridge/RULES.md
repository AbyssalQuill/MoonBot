...
也可回退到 `qq-bridge/state/mode.json`（`{"mode": "default"}`）临时指定。
桥接每 5 秒读取一次，切换即时生效（新会话生效，旧会话可 `/reset` 重建）。
全新安装运行 `scripts/setup-dsh.mjs` 后，DSH 设置与本地兜底默认均为 `default`；若已存在 `state/mode.json` 或 DSH 设置旧值，脚本不会覆盖。
...
- 想彻底关闭机器人：停止桥接进程（start.bat 窗口 Ctrl+C）或 NapCat。
