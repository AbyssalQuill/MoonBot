...
1. **连接 QQ**：通过 NapCat（OneBot v11 WebSocket）收发 QQ 群/私聊消息。
...
| 内核 | `src/mcp-napcat-safe.js` | 给 DSH agent 用的安全 QQ 工具（只读 + 白名单发送） |
| 内核 | `src/mcp-host-server.js` | 给 DSH agent 用的 NapCat 进程管理（默认禁用启停） |
...
`mcp-napcat-safe.js` 给 DSH agent 暴露：
...
- `napcat_status`：只读探活。
- `start_napcat` / `stop_napcat`：默认禁用，需 `config.json` 设置 `napcat.allowProcessControl: true`，且仅在 `closed-agent` 模式下可用。
...
| `napcat.wsUrl` / `httpUrl` | OneBot WebSocket / HTTP API 地址；`httpUrl` 不要填 WebSocket 端口，否则会报 HTTP 426 |
| `napcat.accessToken` | OneBot 鉴权 token，未配置留空 |
...
6. **进程控制**：`start/stop_napcat` 默认禁用；即使开启，也仅允许在 `closed-agent` 模式下调用。
