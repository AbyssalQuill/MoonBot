...
`qq-bridge` 仓库本身包含桥接、控制台和插件，但 **DSH 端的两个聊天模式（`qq-chat` / `default`）以及 MCP 挂载** 不在仓库根目录，需要通过本说明安装到目标设备的 DSH 环境中。
...
2. **安装依赖**：
   ...
   > `postinstall` 会自动修补 `旧版网关 SDK` 的 ESM 打包 bug。
...
3. **创建配置文件**：
   ...
   - `napcat.wsUrl` / `httpUrl`（例如 `ws://127.0.0.1:3001` / `http://127.0.0.1:3000`，分别对应 OneBot WebSocket 与 HTTP API 端口）
   - `napcat.accessToken`
   - `ownerQQ`
...
- **发送消息报 HTTP 426（Upgrade Required）**：说明 `config.json` 里的 `napcat.httpUrl` 指向了 **WebSocket 端口**。`httpUrl` 必须是 OneBot 的 **HTTP API 地址**（例如 `http://127.0.0.1:3000`），而 `wsUrl` 才是 WebSocket 地址（例如 `ws://127.0.0.1:3001`）。请在 NapCat WebUI 的 OneBot 配置里分别确认 HTTP 和 WebSocket 的端口。也可以运行诊断脚本：
   ```bash
