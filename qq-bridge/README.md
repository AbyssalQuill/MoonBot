...
```
QQ 消息 ──► NapCat（OneBot v11 WS）──► 本桥接进程 ──► DSH Web API (127.0.0.1:3080/api)
                                                ▲                      │
                                                └── agent 回复/提问/审批 ┘
```
...
- **QQ 侧**：`旧版网关 SDK` 的 `NapCatWebSocketClient`（OneBot v11 WebSocket 客户端，自动重连）
...
- **agent 自主收发 QQ**：DSH 的 MCP 客户端（`~/.dsh/profiles/web/cordis.patch.yml` 配置）接入三个 MCP server：
  - `napcat`（桥接自带 `src/mcp-napcat-safe.js`）：QQ 动作**安全子集**（查状态/查群/查消息/发消息，发送强制白名单；发送工具支持可选 `replyToMessageId` 引用回复）
...
- **性格定制**：QQ 会话统一使用二代 `default` agent preset（`~/.dsh/.agent-presets/default/agent.cordis.yml`）；**角色扮演**是可选机制——由控制台或管理端设置 `state/current-role.json` 注入（群友无法更改）
- **本地控制台**：桥接自带 Web 控制台 `http://127.0.0.1:3100`——启停二代 AI、设置角色、静默开关、查看活动日志、修改管理员/控制台令牌，全部即时生效；访问需要令牌（`config.json` 的 `consoleToken`，未配置时自动生成并打印在启动日志；控制台内可手动修改或重新生成）
- **运行模式（唯一）**：
  - `default`（二代仿真，运行 `setup-dsh.mjs` 后 DSH 默认）：文本不自动转发，AI 通过 `qq_get_unread_messages` / `qq_send_message` 等工具自主看消息、发言、等待、设置唤醒/潜水；DSH 端使用 `default` preset（历史 chat/closed-agent/一代 reserved 模式已删除）
- **交互增强**：
...
```bash
npm install        # 安装依赖（postinstall 会自动修补 旧版网关 SDK 的 ESM 打包 bug）
```
...
| `napcat.httpUrl` | OneBot **HTTP API** 地址（如 `http://127.0.0.1:3000`）；不要填 WebSocket 端口，否则会报 HTTP 426 |
| `napcat.accessToken` | OneBot accessToken，未配置留空 |
| `napcat.launcherPath` / `homeDir` | NapCat 启动脚本与安装目录（供 agent 自动启动/停止） |
| `agentPreset` | QQ 会话使用的 DSH agent preset，默认 `qq-chat`（改性格见下文） |
| `socialV2.agentPreset` | `default` 模式使用的 DSH agent preset，默认 `default` |
...
桥接和控制台能跑起来还不够，DSH 端还需要安装两个聊天 preset（`qq-chat` / `default`）并挂载 MCP：

```bash
node scripts/setup-dsh.mjs
```

> 全新环境下脚本会把 DSH 默认模式设为 **`default`（二代仿真）**，并创建本地 `state/mode.json` 兜底；这样 AI 使用 `qq_send_message` 等工具收发消息时，DSH 会自动使用 `default` 模式。如果本机已存在旧的 `state/mode.json` 或 DSH 设置值，脚本会保留不覆盖，可手动在 DSH 设置页的 `qq-mode` 卡片切换。

详细步骤见 **[docs/DSH_SETUP.md](docs/DSH_SETUP.md)**。

## 完整启动流程（从零开始）

共 5 步，DSH 和桥接都已就绪，缺的只是 NapCat 本体：

1. **DSH**（已运行，无需操作）
   确认 `http://127.0.0.1:3080` 能打开即可。

2. **下载并解压 NapCat**
   - 下载：<https://github.com/NapCat/NapCat/releases/latest> 选 `NapCat-v<版本>-win-x64.zip`（完整版，自带 Node 运行时；Lite 版需本机 Node 22.13+）
   - 解压到任意目录（例如 `C:\NapCat`），双击 `launcher.bat`
...
5. **启动桥接**
   ...
   ```
   看到 `NapCat 已连接` 即成功；然后 QQ 上给机器人账号发条消息测试。
...
qq-bridge/
  ...
  scripts/              # 测试/运维脚本（含 postinstall 的 patch-napcat-sdk.mjs）
    patch-napcat-sdk.mjs  # 修补 SDK 的 ESM 打包 bug（postinstall 自动执行）
  state/                # 运行时数据（不入库）
```
...
- `旧版网关 SDK` 的 npm 发布版存在 ESM 扩展名 bug，本仓库通过 postinstall 补丁修复（见 `scripts/patch-napcat-sdk.mjs`）

## 合规提醒

NapCat 是独立第三方项目，与腾讯/QQ 无隶属关系，仅供学习与技术研究；使用前请阅读其 EULA 与《QQ 用户协议》。
