// 测试桥接自带的 napcat-host MCP server（status/start/stop）
...
try {
  ...
  console.log('✅ napcat-host MCP 连接成功');
  const tools = await client.listTools();
  console.log('工具:', tools.tools.map((t) => t.name).join(', '));
  const status = await client.callTool({ name: 'napcat_status', arguments: {} });
  const text = status.content?.[0]?.text ?? JSON.stringify(status);
  console.log('✅ napcat_status:', text);
  process.exit(0);
} catch (error) {
