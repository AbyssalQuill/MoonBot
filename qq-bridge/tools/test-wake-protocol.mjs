...
check('A5b 首轮 [Now] 到分钟 + epochMs（精确靠 epochMs，正文不带秒）', /\[Now\] \d{4}-\d{2}-\d{2} 周[一二三四五六日] \d{2}:\d{2} \(Beijing, epochMs=\d{13}\)/.test(first), JSON.stringify(first.split('\n').filter((l) => l.startsWith('[Now]'))));
...
check('B5b 哨兵轮也带 [Now]（分钟精度，主人定稿"不需要带秒"）', /^\[Now\] \d{4}-\d{2}-\d{2} 周[一二三四五六日] \d{2}:\d{2}$/m.test(sentinel), JSON.stringify(sentinel.split('\n').filter((l) => l.startsWith('[Now]'))));
...
/* 【2026-09-20 定稿】唤醒要带 [Now]（哨兵轮以前完全没有时间行），格式到分钟：
 *   首轮/完整正文：[Now] 2026-09-20 周日 19:15 (Beijing, epochMs=1789902923920)
 *   哨兵轮（绝大多数唤醒）：[Now] 2026-09-20 周日 19:15
 * 精确靠 epochMs（与 memory.db 的 ts_ms 同单位），正文里不铺秒 —— 主人明确"不需要带秒"。
 * 完全没有时间行时模型算不准"这条多久之前的"，只能多花一步调 qq_get_recent_messages。 */
check('C2b 哨兵轮/空唤醒带 [Now]（分钟精度）', /^\[Now\] \d{4}-\d{2}-\d{2} 周[一二三四五六日] \d{2}:\d{2}$/m.test(emptyWake), JSON.stringify(emptyWake.split('\n').filter((l) => l.startsWith('[Now]'))));
