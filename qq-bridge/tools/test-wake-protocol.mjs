// 回归测试：唤醒规则搬进系统提示词 + 正文只留数据行 + 去掉 ➤ 哨兵符号 + 【令牌】标签改成 [Token]
//   2026-09-12 起的设计约定：
//     ① 每一种唤醒类型的规则都提前写进系统提示词（persona），之后唤醒注入不再重复带规则；
//     ② 注入只保留 "[Token] …" + "[Mid-turn] N new message(s)…" 这种数据形态；
//     ③ 去掉正文里的 ➤ 哨兵符号；④ 【令牌】改成英文标签 + 英文方括号 [Token]。
//
// 这个测试守两件事，缺一不可：
//   A. 注入侧：正文里没有规则散文 / 没有 ➤ / 没有【令牌】，且体积明显下降；
//   B. 提示词侧：被搬走的每条规则确实在 preset 里（搬走了，不是删了），否则模型会失去规则。
//
...
fs.writeFileSync(path.join(sandbox, 'config.json'), JSON.stringify({
  ...
  // 唤醒概率取 config 里的 recommendedProbability：测试里固定 0.42，方便断言 [WakeRef] 行
  social: { enabled: true, steerEnabled: true, wake: { recommendedProbability: 0.42 } },
  // 访问控制：唤醒调度会先过白名单（沙箱里必须显式放行 owner 私聊，否则 wake 直接被跳过）
...
/* 2026-09-24：首轮正文里的 [Now] 时间行已整行去掉（当时反馈"那个不准"）：它是拼装时刻，
 * 模型真正动手时已经过去一段时间，拿它当"现在"会答错。现在改由 get_time 工具现取（北京时间到分钟）。 */
check('A5b 首轮正文不再注入 [Now] 时间行（改由 get_time 工具现取）', !/\[Now\]/.test(first) && !/epochMs=/.test(first), JSON.stringify(first.split('\n').filter((l) => l.startsWith('[Now]') || l.includes('epochMs'))));
...
check('B5b 哨兵轮也不再带 [Now]（正文里一个时间数字都不留）', !/\[Now\]/.test(sentinel), JSON.stringify(sentinel.split('\n').filter((l) => l.startsWith('[Now]'))));
...
/* 2026-09-19：插话概率等所有概率都要在运行时真正生效。
 * [WakeRef] 行必须每轮都在（首轮 + 哨兵轮），并且写着配置里的那个数字 ——
 * 否则模型拿不到它，就会一直沿用自己上一轮拍的概率，在界面上改等于没改。 */
...
/* 2026-09-24：空唤醒/哨兵轮同样不再带 [Now]（原格式：`[Now] 2026-09-20 周日 19:15`）。
 * 理由与 A5b 相同：正文里的时间只是拼装时刻，会过期；模型要"现在"就调 get_time。 */
check('C2b 空唤醒/哨兵轮也不带 [Now]', !/\[Now\]/.test(emptyWake), JSON.stringify(emptyWake.split('\n').filter((l) => l.startsWith('[Now]'))));
...
const presetPath = path.join(REPO, 'dsh', 'agent-presets', 'qq-chat', 'agent.cordis.yml');
...
const mustHave = [
  ...
  // 2026-09-20：正文不许显式换行 / 颜文字有条件发 / 数组当正文是硬失败 / 代码不分段
  ...
  // 2026-09-13：默认人设改为 "You are a helpful assistant"，不再用「小鲸鱼」人设
  ['主人段落不再有撒娇/顺从注册', null],
];
...
/* ── G. 人设注入（2026-09-12 反馈过："我上传的人设不是你注入的人设而是精简版"）────
...
/* ── H. 改插话概率 → 正在跑的会话立刻跟上（只动 source=owner 的）────────────
 * 2026-09-19：插话概率等所有概率都要在运行时真正生效。这一节的现场是：
 ...
 * 于是"改了没反应"。现在给概率带来源标记：owner 的跟着配置走，model 的保留模型的选择。 */
{
  ...
  /* 2026-09-19 改口径：保存的概率现在覆盖所有会话，包括"模型自己定过"的那些
   * （当时的反馈："我调的 0.15，唤醒词里带的好像是 0.08" —— 旧口径保留了模型的值，看起来就是没生效）。 */
 ...
 * 2026-09-19：一个会话永久使用，但别让上下文堆积，省掉切新会话的首轮 token。
 * 这里断言的是一处开关管住全部轮换路径：
