/* 用"送达/幽灵"当读数，逐个变量找出哪个字段让 QQ 拒收。
 *
 * 为什么可以放心跑：幽灵是对方完全不可见的（服务端没收，对方什么都看不到），
 * 所以失败的组合不产生任何打扰；真正送达的那几条才会显示出来 —— 那正好就是要的结果。
 *
 * 已知：
 *   送达：05:21:08 那张 tuwen 卡送达了，它比手写的卡多了 `extra.uin` 与 `meta.news.uin`
 *      （值 3929684621，和签名服务输出里的 uin 一致），而它的 jumpUrl 不带 `&`。
 *   幽灵：08:33/08:40 的四张全幽灵：没有 uin，jumpUrl 是 `i.y.qq.com/v8/playsong.html?...&...&...`。
 *
