/* 直接看**消息本身的发送状态**（`sendStatus` / `sendStatusExtInfo`），
 * 判断"到底发出去了没有" —— 这比 OneBot 回执和本地库都可信。
 *
 * 为什么要它：主人说私聊里什么都没有，但 NapCat 的本地库里有这些卡片。
 * 本地库写了 ≠ 送达：QQ 客户端会先把消息落本地（乐观写入），**服务端拒收时**本地仍留记录。
 * QQ 自己的 RawMessage 里带 `sendStatus`（实测：2 = 已发送成功；其它值是失败/发送中），
 * 拿它跟主人**真机分享进来的卡**（一定送达）对照，就能一眼看出我们的卡是不是被拒了。
 *
