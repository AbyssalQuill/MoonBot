...
//
// 现在：首次连接失败带退避重试；预算用完也不退出，保留控制台并后台每 15s 再试。
// 本测试在临时沙箱里跑（复制 src + 自造 config.json + 把 HOME 指到沙箱），
...
fs.writeFileSync(path.join(sandbox, 'config.json'), JSON.stringify({
  ownerQQ: '10001',                          // 沙箱用的假 owner QQ（loadConfig 要求合法正整数）
...
// ⑦ 用时间差判断（比固定字符窗口可靠）：放弃 NapCat 到控制台起来之间不能隔很久
