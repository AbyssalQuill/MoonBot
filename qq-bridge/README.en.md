...
```
QQ messages ──► NapCat (OneBot v11 WS) ──► qq-bridge ──► DSH Web API (127.0.0.1:3080/api)
                                                    ▲                      │
                                                    └── agent replies / questions / approvals ┘
```

- **QQ side**: `旧版网关 SDK` provides the OneBot v11 WebSocket client.
...
- MCP tools for reading group history/members, sending messages, replying with quotes, and (in `default`) full simulated-group-friend tooling.
...
```bash
npm install        # postinstall automatically patches the 旧版网关 SDK ESM packaging bug
```
...
| `napcat.wsUrl` | NapCat OneBot **WebSocket** URL (e.g. `ws://127.0.0.1:3001`) |
| `napcat.accessToken` | OneBot access token, leave empty if not configured |
| `napcat.httpUrl` | OneBot **HTTP API** URL (e.g. `http://127.0.0.1:3000`); do not point this at the WebSocket port or you will get HTTP 426 |
...
The bridge and console can run without extra DSH setup, but the two DSH chat presets (`qq-chat` and `default`) and the MCP servers must be installed into DSH once per machine:
...
- Process control for NapCat (`start_napcat` / `stop_napcat`) is disabled by default and only allowed in `closed-agent` mode when explicitly enabled.
...
```bash
npm run self-test       # DSH-side link test, no QQ/NapCat required
npm run test-md
npm run test-wait
npm run test-vision
...
NapCat is an independent third-party project and is not affiliated with Tencent/QQ. This project is for learning and technical research only; please follow the relevant terms and the QQ User Agreement.
