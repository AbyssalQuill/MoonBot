/* 回归测试：【目标机环境自愈】——主人 2026-09-12 要求：
 *   "部署功能要求自动检测服务器缺失环境，比如 npm、node 这些，没有就自动安装别报错停止，别留我安装。"
 *
 * 用一个**假的 SSH 连接**（模拟一台全新 Ubuntu 的 shell 行为）驱动真实的 ensureTargetEnv()，
 * 覆盖四种现场，其中 ② 就是主人真实踩到的那个坑（有 node、缺 npm → 旧实现直接跳过安装、后面必失败）：
