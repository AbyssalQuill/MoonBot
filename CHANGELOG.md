...
- **三份技术文档重写**（技术／学术文体，逐条带相对路径与常量，未知项显式标注；行数均按"含空行"口径统计）：`docs/ARCHITECTURE.md` 628 → 1,490 行 / 134 KB（新增安装布局与路径解析、通信面全表、首启与鉴权、NapCat 守护、SSH 克隆部署深挖、DSH 集成、打包分发、可靠性与不变量、待确认清单）；`docs/CAPABILITIES.md` 457 → 1,764 行 / 201 KB（§0 总表 52 条能力 + §13.5–13.9 与 §17–§18 的 DSH／管理端／部署面）；`docs/COMPACTION-MATH.md` 540 → 1,676 行 / 106 KB（符号系统、分块与递归摘要、成本泛函与触发判据、最优性、复杂度、实验口径，新增 §4.5 生产复算记录）。
...
- **完全投入角色扮演**：内置发言规则新增 `[Roleplay]` 一节，写明"人设就是你本人"——每条消息、每个话题、每一轮都在角色里，不夹半出戏的旁白；换人设即换人，不把上一个角色的自称、口癖、称呼、世界观带过来；同一时间只认这一轮注入的那个人设。同一条约束也写进系统提示词（`agent-presets/default` 与 `qq-chat`），群聊与社交会话一并生效。

### 变更与不兼容

- **删掉"连发"工具 `qq_send_burst`**：模型不再有"一次连发多条"的独立入口，减少一口气补完式的刷屏。发多条 Message 仍然可以——`qq_send_message` 的 `messages` 数组走的正是同一条发送链：同样的分条节奏、频率上限、敏感词与重复拦截、发送后的质量软提醒，能力没有减少。同时移除控制台的 `/api/social/send-burst` 端点、`social.tools.sendBurst` 开关（管理端「连发多条」那一格）、工具发送正则里的 `send_burst` 与参数修复名单里的同名条目；`sendMessages` / `computeGaps`（分条节奏）等底层能力原样保留。
- **NapCat 启动方式收敛为一条**：删除 VBScript 启动器（`.vbs` 生成、引擎探测、按需功能安装、提权路径与四级兜底全部移除），现在只用 `powershell -WindowStyle Hidden -Command "Start-Process … -WindowStyle Hidden"`。黑框、QQ 窗口与任务栏按钮在整条进程链（启动器 → NapCatWinBootMain → QQ）上都不出现；系统自带 PowerShell 始终存在，因此不再依赖脚本引擎、不再需要装按需功能、也不会再弹 UAC。
- **内置发言规则重写为角色无关的短句模板**：原规则夹带某一具体人设的示例（例如"我是鲸鱼不是萝莉"）与整节对照样例，切换人设时会把旧角色的口吻带过去。现在只保留必要的可判定描述——发送与引用协议、分段节奏、看场合、表情包用法、去 AI 味的硬线——全部写成**正常大小写的英文短句**（不再用全大写 token 体例），节标题一律用英文中括号（`[Delivery]`、`[Splitting]`、`[Quoting]`、`[Wake]`、`[Meme]`、`[Style]`、`[Room]`），只有示例保留中文（说出去的本来就是中文）。文件从 11.2 KB 降到 6.9 KB，管理器点「恢复默认发言规则」写回的模板与 `qq-bridge/speech-rules.md` 逐字对齐（此前两者长期漂移，点一次恢复会把规则覆盖成旧版），上限 12000 字符（`RUNTIME_OVERRIDE_MAX`）不变。
- **清空人设不再写提示语进 persona.md**：以前清空会把一句中文提示（"未启用角色卡：按默认人格与发言规则说话，不要再演任何角色。"）写进人设文件，而这段文字会**原样注入给模型**，等于每轮都在叮嘱"别再演了"。现在清空就是真的清空，管理端人设编辑框用灰色占位词 `You're a helpful assistant.` 说明"当前为人设、即默认助手"，占位词只显示、不落盘、不注入。
- **斜杠指令只认英文**：`/portrait` 只保留 `learn | stop | status`（删掉中文别名"画像学习 / 群友画像学习"、中文子命令与 `/portrait start`、`/portraitlearn` 等简写）；`/role` 的清除只认 `clear | off | none | default`（删掉"默认 / 清除 / 关闭"）。未知子命令只回用法文案。管理端「指令速查」与说明文档同步删掉"亦识别…""中英混写已不再识别"这类兼容备注。
- **工具名单档位改为自定义预设方案**：删掉 off/low/medium/high/extreme 那套档位选择器与全部实测读数（工具个数、字符数、占比），名单一律由用户勾选产生；勾选结果可存为**命名方案**并支持改名、删除、用当前名单覆盖、一键载入（载入即刻同步卡内勾选状态与计数）。方案存于 `social.slimTools.schemes`，写入名单时自动把 `social.slimTools.level` 置为 `custom`（只有该档位下手写名单才会真正生效）。
- **界面字体与按钮动效**：全站说明文字统一为随包自带的可爱圆体（AaCute / AaCute-full），不再回退到系统字体；等宽只保留三处——首页卡片里的日志查看器、`config.json` 编辑区与代码片段，等宽字体也改为随包自带的 JetBrains Mono。所有按钮改为极淡的流动渐变（15 秒一循环，色差压到最小），首页主按钮原先的白色扫光与卡片强反光已调弱或删除；`prefers-reduced-motion` 下全部动画关闭。
- **首页改版**：标题改为英文 `Made a character, kept a friend.`（中文副标题保留）；新增极光背景、标题入场与渐变字、四张卡片错峰入场与悬停微光，全部为纯 CSS 动效。
- **不再附带出厂表情包**：`meme/whale-fanart-001` 已从仓库与安装包载荷中移除，表情包一律由用户自行导入（上传包、角色专属包）。相关工具路径、SSH 部署软链与「从本机复刻」的源探测均改为扫描"任意存在的包"，不再写死包名。
- **界面说明文案重写并取消等待态**：管理端各页说明统一为庄重书面语体（去掉口语感叹、卖点词与比喻；键名、命令、数值原样保留），每处按"这是什么 → 怎么用 → 注意点"组织。「拟人默认与安全线」整张卡片、NapCat 令牌卡里的现状说明块（配置文件 / 运行形态 / 涉及文件 / 磁盘现状 / 期望值）与首页英文标语已删除或改写。进入桥页面、连接远端服务器、语音配置页不再出现"加载配置中…""正在读取语音配置…"这类阻塞等待：界面先按出厂默认值渲染，数据到达后就地填充，读取失败只给一行原因与重试。

### 新增能力

- **模型可主动查当前时间（`get_time`）**：新增 MCP 工具 `get_time`，返回北京时间（UTC+8）到分钟，与消息行、工具结果同一口径，字段含 `beijing / date / weekday / time / hour / timezone / epochMs`。唤醒正文不再自带时间行（原因见下条），因此任何与时间有关的回答都以这次调用为准。工具是本地读取系统时钟，不发网络请求、不碰 NapCat，schema 约 450 字符，不占检索预算。
- **深夜休息提醒**：系统提示词新增 `OWNER_REST` 规则——在私聊里若北京时间已过约 23:30（或凌晨 1 点后对方仍在线），模型可以收尾一句提醒休息，或在聊天安静时主动发一条；每个自然日最多一次，做过一次就写进记忆并在下次先查记忆，对方说忙/不用就立刻停止。群聊与陌生私聊不适用。

### 变更与不兼容

- **去掉唤醒正文里的时间行**：原 `[Now] 2026-09-20 周日 19:15（epochMs=…）` 是**拼装正文那一刻**的时间，模型真正动手时已经过期，同一条上下文里堆着好几轮旧 `[Now]` 时还会取到更早的一条，导致答错时刻。现在该行整体移除，`[Status]` 保留（未读、最后说话人、潜水模式等是过去的事实，不会过期），"现在几点"改由 `get_time` 现取。
- **agent preset 合并为单份 `qq-chat`**：仓库与隔离 home 此前同时存在 `default` 与 `qq-chat` 两份预设，显示名同为「QQ 聊天角色」，在 DSH 预设列表里重复出现。现在只保留 `qq-chat` 一份（内容取线上实际生效的那份，原 `qq-chat` 中已过时的提示词正文废弃，`qq-tool-restrict.mjs` 取带实测结论的版本），配置默认值（`agentPreset` / `slang.learnerPreset`）与人设合成路径（`lib/preset-compose.js`）一并改名。注意：DSH 的预设只在建会话那一刻绑定，**已经在跑的会话不受影响**，要让它拿到新预设名需要等会话轮换（或先关掉「永久会话不轮换」）。
- **「添加群」按钮改小并改用主色**：此前与群号输入框等宽等高（196×38、浅粉 `btn-soft-primary`），按钮上只有三个字显得空。现在输入框保持 196×38，按钮收成 30px 高的紧凑尺寸并换成与「添加服务器」相同的 `btn-primary`（红/pink 主色），圆角、阴影、悬停态完全沿用既有按钮样式。
- **说明文字统一为可爱圆体**：语音配置「预置地址」说明与工具压缩代理卡首段说明里含 `<code>` 片段，此前被全站等宽规则压成 JetBrains Mono，读起来"一半可爱一半代码"。这两处改用 `.cute-note`（含子元素），代码编辑器与日志查看器仍保持等宽。
- **界面滚动条全部隐藏**：所有滚动容器不再绘制滚动条（标准属性与 `::-webkit-scrollbar` 成对覆盖），滚动本身不受影响——滚轮、触摸板、键盘与拖动照常可用。`select` 一类此前的滚动条规则一并被覆盖。

### 内部与工程

- 新增 `tools/sync-speech-template.mjs`：把发言规则模板重新生成为 `qq-bridge/speech-rules.md` 的逐字副本，`--check` 可校验一致性（防止两处再次漂移）。
- 仓库与载荷里移除出厂表情包后，安装包与运行目录同步减小约 7 MB。
- 代码注释统一整改：去掉"某人要求"式的转述口吻与日期装饰壳（`【2026-09-24 主人要求】` → `2026-09-24：`）、去掉注释里的表情符号与加粗强调，只保留可验证的事实（数字、实测结论、报错原文、设计原因）。
