...
- **工具描述支持"压缩档"，管理端一键切换**（主人要求"MCP 压缩工具调到 high 档、压缩到 8.6%、并支持管理端切换"）。这一条是账单上最大的一刀：实测一次请求里 **工具 JSON 描述 74,422 字符 ≈ 23.3k token，占整份提示词的约 87%，而且每一步都重发一次**。以前只有两张手写白/黑名单，能省但**没人知道省了多少**，写错名字还会静默失效。现在：
  ...
  - 当前实测（本机 69 个工具）：`low` 保留 52.6%、`medium` 32.9%、`high` **8.5%**（≈2,018 token/步，省 91.5%）—— `high` 就是主人要的"8.6% 档"：只留「说话 / 引用 / 收尾 / 看未读」这个最小闭环；
  ...
  - 复算/挑名单用 `qq-bridge/tools/tool-schema-meter.mjs`（它会真的起一次 MCP server 逐个量）。
