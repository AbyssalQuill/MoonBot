...
- **唤醒正文的时间只到分钟，模型想算"多久之前"就得再花一步**：`[Now]` 现在带**精确到秒**的时间戳与 epoch 毫秒（形如 `[Now] 2026-09-20 周日 19:15:23 (Beijing, epochMs=1789902923920)`），消息行的时间戳也一并精确到秒，**与 `memory.db` 的 `chat_messages.ts / ts_ms` 完全同形**（主人要求"和 sqlite 里一样"）。以前只有 `HH:MM`，"这条多久之前的 / 你多久没回我"算不准，模型只能多调一次 `qq_get_recent_messages` —— 而每多一步就重发一整份系统提示词与工具表。
...
- `tools/test-dsh-compaction.mjs` 的阈值门禁改成两头夹：缺省阈值 × 1M 必须落在 [8 万, 10 万] token（低于 8 万 = 每步压缩，高于 10 万 = 一句话 1 分钱）、压缩后离阈值至少留 2.5 万 token、事故值 0.12 不再允许做缺省、低于下限的 0.06/0.02/0.005 必须被夹回、出厂 `config.example.json` 与代码缺省必须一致。
