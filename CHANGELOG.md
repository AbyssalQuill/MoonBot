...
- **模型漏写引号 → 整条消息发不出去（"key、messages、images 至少一个不能为空"）**：现场（`state/tool-calls.jsonl`，12:42:32 / 12:42:34 / 12:42:35）三次调用分别是 `{"key":"private:…","messages": 主人这么直接啊 我脸都热了, "token":"…"}`（**字符串值没包引号 → 不是合法 JSON**）、同样再试一次、最后 `{"key":"…","token":"…","messages":"主人这么直接啊 我脸都热了"}` 才成功。DSH 对工具参数是宽松解析，非法成员会被**整个丢掉** → 到达发送端点时 `messages` 为空，模型只看到"至少一个不能为空"，于是**原样重试**（白烧两步 ≈ 1.4 分）。两处一起修：① 发送端点的 400 回执现在写明最可能的原因与确切写法（"字符串值必须用**双引号**包起来；裸文本或单引号都不算 JSON，会被丢弃"），工具层会把这段原文返回给模型，一次就能改对；② 预设 `[TOOLS]` 第 2 条写明 **Arguments are JSON: every string value must be wrapped in DOUBLE quotes**（文本里出现 `"` 要转义）。回归测试 `tests/outbound-send-integration.test.js` 新增 E2E6/E2E7（7/7 通过）。
- **"改成单引号"这件事本身不成立**：MCP 的工具参数按协议就是一个 JSON 对象，单引号在**解析层**就不合法，换成单引号只会比双引号更早失败（实测模型用双引号写对的那次一次就发出去了）。所以没动引号风格，而是把"漏引号"这个真因堵在提示词与回执里。
