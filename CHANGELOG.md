...
> **追加修复（同一次安装包内，2026-09-22，共十五条）**：下面十五条是打包后被主人实测报出来的，前五条已修进这一版的两个安装包，后面十条是当晚第二到第八轮的线上修复（把图发出去四轮 + 私聊卡住的根因一轮 + 文件类型标记与 NapCat 界面鉴权各一轮 + 群里认人认主人与人设切换一轮 + 收尾那句 OK 与表情包参数各一轮）。
>
...
> - **"模型不是每次都是 OK"**（第八轮，主人报"无伤大雅但是可以强化"）。现象：模型用发送工具把话发完之后，收尾又单独发一条 `OK`（或"好了 / 已发送"），主人那边看到的就是「正经回复」+「一句 OK」两条。上一轮加的 `[RULES] 13 NO_TRAILING_REPORT` 只是"请它别这样"，模型偶尔还是照发 —— 这一轮补的是**不依赖模型听话**的那一半：桥侧新增判定器 `src/lib/ack-text.js`（`isAckOnlyText`），发送端点（`src/core/console-server.js` 的 `/api/send/group|private|reply`）在真正发出去之前多问一句"本回合已经发过气泡了吗"（`session-state.js` 的 `turnHasBubble`：`sendToolSucceededSessions` / `pendingTurnOutbound` / `lastAiReplyAt` 三个判据），已发过 + 整条正文就是一句回执 → 不发，回给模型 `{ok:true, sent:0, ackSuppressed:true, note:'…请直接结束本回合，不用补别的话'}` —— **用 ok 回而不是报错**，否则它会当成失败去重试、或者换一句别的话再发一次。闸门刻意开得极窄：整条正文必须本身就是回执（英文 `ok / okay / done / got it / finished`，中文 好的/好嘞/好滴/好啦/好了/已发送/已发出/发送成功/发送完毕/发送完成/已经发送/已回复/已处理/已完成/完成），长度上限 12 字，`**OK**`、`` `OK` `` 这类 Markdown 包裹照认、句末标点不影响，而 **"嗯 / 好 / 在 / 收到 / 行"这种本身就是正常回复的短句一律不拦**（首条回复更不拦：`turnHasBubble` 为假时闸门根本不开）。回归 `qq-bridge/tests/ack-suppress.test.js`（39 项，含"正常回复绝不能被误伤"一整组）。
