...
- **工具描述支持"压缩档"，管理端一键切换**（主人要求"MCP 压缩工具调到 high 档、压缩到 8.6%、并支持管理端切换"）。这一条是账单上最大的一刀：实测工具 JSON 描述**占整份提示词的约 87%，而且每一步都重发一次**（本机出厂配置：服务端共定义 90 个工具、schema 合计 **89,681 字符 ≈ 28k token/步**；线上主会话的 request/header 里量到的是 72,692 字符 ≈ 22.7k）。以前只有两张手写白/黑名单，能省但**没人知道省了多少**，写错名字还会静默失效。现在：
  - 档位写在 `social.slimTools.level`：`off`（全部注册，默认）/ `low` / `medium` / `high` / `custom`（继续用老名单）；
  - **百分比是桥实测的**，不是界面上写死的：`mcp-napcat-safe.js` 在注册每个工具时把它们进请求体的三样（名字 / 描述 / 参数表）加总，注册完写 `state/tool-schema-stats.json`，管理端「工具 schema 精简」卡直接读它，显示"当前档位实际注册了几个、多少字符、占百分之几、省了多少"；
  - 当前实测（分母 = **不裁剪时全部已定义工具**）：`low` 保留 **44.5%**、`medium` **27.9%**、`high` **7.2%**（≈2,018 token/步，省 92.8%）—— 主人要的"8.6% 档"不仅达到、还更低一档：`high` 只留「说话 / 引用 / 收尾 / 看未读」这个最小闭环；
  - 手写白名单优先级最高（明确点名要什么就只给什么），`qq_status` 任何档位都保留（198 字符的自检通道）；
