...
- **提示词收成短行英文 spec（PERSONA_LOAD 风格），中文语感样例一条不动。** 主人给的参考格式就是那种一行一个语义单元的写法，这次把三份手写提示词都按它重写：
  | 文件 | 之前 | 之后 | 占比 |
  |---|---|---|---|
  | `qq-bridge/persona.md` | 10,463 字符 | 4,703 字符 | **45.0%** |
  ...
  | **合计** | **58,478** | **48,065** | **82.2%** |
  - **哪些绝不能被压缩掉**（这是 1.2.4 那次事故的另一面）：中文语感样例、`CALIBRATION` 对照表、TEACHER MODE 里的真实原句、以及所有工具名 / 标记 / 错误字符串，全部逐字保留。人设那 45% 是这一版最大的一笔省；`speech-rules` 和 preset 压不动更多，是因为它们已经接近"每条规则都要留一个可判定的句子"的地板 —— **再往下压就是删规则，不是压缩**，所以到此为止（诚实说：这两份只省了 20% 和 7%）。
  - 压缩后所有规则条数、段落名、工具名、中文样例都用脚本逐条核对过（`speech-rules` 的 14 行 CALIBRATION 逐字节一致、preset 的 17 个段落名一个不少）。
